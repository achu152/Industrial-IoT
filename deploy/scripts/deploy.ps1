        ...
        if ([string]::IsNullOrEmpty($script:branchName)) {
            try {
                ...
                throw "This script requires *git* to be installed and must be run from " + `
                    ...
                    "See the deployment documentation at https://github.com/achu152/Industrial-IoT " + `
...
Function Select-RegistryCredentials() {
    ...
    if ([string]::IsNullOrEmpty($script:acrRegistryName) `
     ...
     Write-Host "Test ::: script:acrSubscriptionName: $($script:acrSubscriptionName) And script:acrRegistryName : $($script:acrRegistryName)"
